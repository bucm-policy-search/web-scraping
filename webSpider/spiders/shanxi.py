...
class HCOSP(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in range(1, int(page) + 1):
            ...
            logging.warning("Start to crawl: {}".format(url))
    ...
    def content_url(self, response):
        ...
        try:
            ...
            item["publishingDate"] = re.search(
                r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}:\d{1,2}:\d{1,2})", publishingDate
            ).group(0)
            ...
            if attachment_list_a is not None:
            ...
            logging.error("Error url: {}".format(response.url))
